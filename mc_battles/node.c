...
Lyric* lyric_create(const char* mc_name, const char* content) {
    Lyric* lyric   = (Lyric*) calloc(1, sizeof(Lyric));
    lyric->mc_name = (char*)  calloc(1, strlen(mc_name) + 1);
    lyric->content = (char*)  calloc(1, strlen(content) + 1);
...
Round* round_create(const char* winner_name, Array lyrics) {
    Round* round       = (Round*) calloc(1, sizeof(Round));
    round->winner_name = (char*) calloc(1, strlen(winner_name) + 1);
...
Battle* battle_create(const char* name,
    ...
    battle->name        = (char*)   calloc(1, strlen(name) + 1);
    battle->mc1_name    = (char*)   calloc(1, strlen(mc1_name) + 1);
    battle->mc2_name    = (char*)   calloc(1, strlen(mc2_name) + 1);
    battle->winner_name = (char*)   calloc(1, strlen(winner_name) + 1);
    battle->date        = (char*)   calloc(1, strlen(date) + 1);
